...
const AwardsSection = ({
  ...
  awards = [
    ...
    },
    {
      id: 2,
      title: "J.D. POWER",
      subtitle: "In 2023, CHERY RANKS FIRST IN THE INITIAL QUALITY STUDY (IQS) AMONG CHINESE DOMESTIC BRANDS.",
      description: "According to J.D. Power's various automotive index studies for 2023, Chery holds the top position in the IQS (Initial Quality Study) among China's domestic brands. Additionally, its vehicle series, including the TIGGO 8, TIGGO 7 and TIGGO 4 have each achieved first place in their segments.",
      bgColor: "bg-[#e9e2d8]",
  ...
  return (
    <section className="w-full h-full bg-white py-12 md:py-16">
      <div className="max-w-7xl mx-auto px-4 md:px-8">
        {/* Section Title */}
        <h2 className="text-4xl md:text-5xl font-bold text-[#6B5A44] mb-8">{sectionTitle}</h2>
        ...
        <div className="relative flex flex-row w-full overflow-hidden">
          {awards.map((award) => (
            <motion.div
              ...
              onClick={() => setExpandedSection(expandedSection === award.id ? null : award.id)}
            >
              <div className="p-8 md:p-10 lg:p-12 h-full flex flex-col justify-center">
                <h3 className={`text-2xl md:text-3xl font-bold ${award.textColor} mb-6`}>{award.title}</h3>
                {award.subtitle && (
                  <h4 className={`text-xl md:text-2xl font-semibold ${award.textColor} mb-6`}>{award.subtitle}</h4>
                )}
                <motion.div
                ...
                >
                  <p className={`${award.textColor}/90 text-base md:text-lg leading-relaxed`}>{award.description}</p>
                </motion.div>
              </div>
            </motion.div>
          ))}
          ...
          <motion.div
            ...
            transition={{ duration: 0.5, ease: "easeInOut" }}
            onClick={() => setExpandedSection(expandedSection === actionButton.id ? null : actionButton.id)}
          >
            <div className="p-8 h-full w-full flex flex-col justify-center items-center">
              ...
              >
                <span className="relative text-xl md:text-2xl font-medium mb-3">{actionButton.text}</span>
                <motion.svg
              ...
              <motion.div
                ...
                transition={{ duration: 0.3 }}
              >
                <div className={`${actionButton.textColor}/90 text-base md:text-lg leading-relaxed`}>
                  <p className="mb-4">Discover more about our achievements and recognition in the automotive industry.</p>
              ...
              <div className="absolute right-8 top-0 bottom-0 h-full">
                <div className="h-full flex items-center">
                  <p className={`${actionButton.textColor}/30 text-xl transform -rotate-90 whitespace-nowrap`}>
                    Explore More
                  </p>
                </div>
              </div>
